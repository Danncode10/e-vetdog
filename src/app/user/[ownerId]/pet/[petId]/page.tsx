...
export default async function PetRecordPage({ params }: { params: Promise<{ ownerId: string; petId: string }> }) {
  ...
  return (
    <div className="mx-auto w-full max-w-4xl space-y-8">
      ...
      <section aria-labelledby="owners-heading"><h2 id="owners-heading" className="border-b border-border pb-3 text-lg font-semibold text-foreground">Authorized owners</h2><ul className="divide-y divide-border">{pet.owners.map((owner) => <li key={owner.id} className="flex min-h-16 flex-col justify-center gap-1 py-3 sm:flex-row sm:items-center sm:justify-between"><p className="font-medium text-foreground">{owner.fullName || owner.email || "Owner profile"}</p><p className="text-sm capitalize text-muted-foreground">{owner.relationship.replace("_", " ")}{owner.isPrimaryContact ? " · Primary contact" : ""}</p></li>)}</ul></section>
